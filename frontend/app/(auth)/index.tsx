...
const Signup = () => {
  ...
  const [isOtpStep, setIsOtpStep] = useState(false); // Controls which view is visible
  ...
  const handleSignup = () => {
    if (validateForm()) {
      ...
      setTimeout(() => {
        setIsLoading(false);
        setIsOtpStep(true); // Switch to the OTP verification view
      }, 1000);
    }
  };
  ...
  const handleVerifyOTP = async () => {
    ...
    if (otpCode === "123456") {
      ...
      router.push("/(tabs)"); // Navigate to the main part of the app
    } else {
      Alert.alert("Error", "Invalid OTP. Please try again.");
      setOtp(["", "", "", "", "", ""]); // Clear OTP fields on failure
    ...
    <View>
      <View className="items-center mb-10">
        ...
        <Text className="text-lg text-blue-700 text-center px-8 leading-6">
          Connect instantly with QR codes. Simple, secure, and fast messaging.
        </Text>
      </View>
      <View className="bg-white rounded-3xl p-8 shadow-lg">
        ...
        />
        <Text className={`text-red-500 mt-1 h-5 ${errors.firstName ? 'opacity-100' : 'opacity-0'}`}>
          {errors.firstName || ''}
        </Text>
        ...
        />
        <Text className={`text-red-500 mt-1 h-5 ${errors.lastName ? 'opacity-100' : 'opacity-0'}`}>
          {errors.lastName || ''}
        </Text>
        ...
        <TextInput
          ...
          autoComplete="email"
        />
        <Text className={`text-red-500 mt-1 h-5 ${errors.email ? 'opacity-100' : 'opacity-0'}`}>
          {errors.email || ''}
        </Text>

        <TouchableOpacity
          className={`py-4 rounded-xl items-center mt-4 shadow-lg ${isLoading ? "bg-gray-400 opacity-50" : "bg-[#4895ef]"}`}
          onPress={handleSignup}
          disabled={isLoading}
        >
          <Text className="text-white font-bold text-lg">{isLoading ? "Processing..." : "Sign Up"}</Text>
        </TouchableOpacity>
      ...
      <ImageBackground source={bg} resizeMode="cover" style={styles.container}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.container}>
          <ScrollView contentContainerStyle={styles.scrollContentContainer} keyboardShouldPersistTaps="handled">
            {isOtpStep ? renderOtpForm() : renderSignupForm()}
          </ScrollView>
        </KeyboardAvoidingView>
      </ImageBackground>
    </SafeAreaView>
